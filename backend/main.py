...
## Frontend - React
# Updated to include new overlays and custom upload
...
export default function App() {
    ...
    const uploadCustomOverlay = async (event) => {
        const file = event.target.files[0];
        const formData = new FormData();
        formData.append("file", file);
        const res = await axios.post("/upload_overlay/", formData);
        setCustomOverlay(res.data.overlay_path);
    };

    const applyOverlay = async () => {
        const res = await axios.post("/apply_overlay/", {
    ...
    return (
        <div>
            ...
            <input type="file" accept="image/*" onChange={uploadCustomOverlay} />
            ...
            {processedVideo && <video src={processedVideo} controls />}
        </div>
    );
}
